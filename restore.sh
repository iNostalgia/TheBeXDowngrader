...
display_help() {
    echo ' *** The BeX Downgrader ***
  - Made by @iNostalgia -
  - Original script by LukeZGD -
...
set_tool_paths() {
    ...
    if [[ $OSTYPE == "linux"* ]]; then
        ...
        if [[ $(id -u $USER) == 999 || $USER == "liveuser" ]]; then
            ...
            if [[ $(pwd) == "/home"* ]]; then
                ...
                if [[ $(lsblk -o label | grep -c "casper-rw") == 1 || $(lsblk -o label | grep -c "persistence") == 1 ]]; then
                    log "Detected The BeX Downgrader running on persistent storage."
                    live_cdusb_str="Live - Persistent storage"
                else
                    warn "Detected The BeX Downgrader running on temporary storage."
                    print "* You may run out of space and get errors during the downgrade process."
                    print "* Please move The BeX Downgrader to a drive that is NOT used for the live USB."
                    print "* This may mean using another external HDD/flash drive to store The BeX Downgrader on."
        ...
        if [[ $(pwd) == *"/media"* ]]; then
            ...
            print "* If this is the case, try moving The BeX Downgrader to the Desktop or Documents folder."
        fi
    ...
    if [[ ! -d $dir ]]; then
        ...
        "* Re-download The BeX Downgrader from releases (or do a git clone/reset)"
    fi
...
install_depends() {
    ...
    if [[ $platform == "linux" ]]; then
        print "* The BeX Downgrader will be installing dependencies from your distribution's package manager"
        print "* Enter your user password when prompted"
        pause
    fi
    ...
    if [[ $distro == "arch" ]]; then
        ...
        print "* The BeX Downgrader will be installing dependencies and setting up permissions of tools"
        print "* Enter your user password when prompted"
        pause
        sudo xattr -cr $dir
...
device_get_info() {
    ...
    if [[ -z $device_proc ]]; then
        ...
        print "* ECID: $device_ecid"
        echo
        error "This device is not supported by The BeX Downgrader."
    fi
...
ipsw_preference_set() {
    ...
    if [[ $device_target_vers == "3.1.3" && $device_proc != 1 ]]; then
        ...
        print "* I recommend to enable this option to have if you have macOS Mojave or higher."
...
menu_print_info() {
    ...
    print " *** The BeX Downgrader ***"
    print " - Script by @iNostalgia -"
    print " - Original script by LukeZGD -"
    echo
    ...
    device_manufacturing
    if [[ $device_proc == 1 ]]; then
        warn "This device is only partially supported by The BeX Downgrader. Some features may not work properly."
    fi
...
restore_customipsw() {
    ...
    print "* Proceed with caution when restoring to custom IPSWs not made with The BeX Downgrader."
    if [[ $device_newbr == 1 ]]; then
...
main() {
    ...
    case $mode in
        ...
        "baseband" )
            ...
            print "* To stitch baseband to IPSW, run The BeX Downgrader with --disable-bbupdate argument:"
            print "    > ./restore.sh --disable-bbupdate"
        ;;
        "actrec" )
            device_dump activation
            log "Activation records dumping is done"
            print "* To stitch records to IPSW, run The BeX Downgrader with --activation-records argument:"
    ...
    print "* The BeX Downgrader $version_current ($git_hash)"
    echo
}
